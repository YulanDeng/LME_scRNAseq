...
#################################################################################
#NicheNat analysis from cancer cell to monocyte and macrophage (R version 4.0.3)#
#################################################################################
#${workDir} is the working directory
#${ggoFile} is the seurat object for all the cells
#${cancerFile} is the seurat object for cancer cell
#${moFile} is the seurat object for monocyte and macrophage
...
#make the input data
sub.integrated <- subset(ggo.integrated,cells=c(rownames(cancer.integrated@meta.data[as.character(cancer.integrated@meta.data[,"integrated_snn_res.0.25"])=="2",]),rownames(immune.integrated@meta.data[as.character(immune.integrated@meta.data[,"assigned_cell_type_byNC"])%in%c("CD14_monocyte","CD16_monocyte"),])))
DefaultAssay(sub.integrated)<-"RNA"
expression = t(as.matrix(GetAssayData(sub.integrated,assay = "RNA")))
sample_info = as.data.frame(sub.integrated@meta.data,stringsAsFactors =F)
...
cell_type <- rep("unknown",nrow(sample_info))
names(cell_type) <- rownames(sample_info)
inter1 <- intersect(rownames(cancer.integrated@meta.data[as.character(cancer.integrated@meta.data[,"integrated_snn_res.0.25"])=="2",]),names(cell_type))
cell_type[inter1] <- paste0("cancer_",
as.character(cancer.integrated@meta.data[inter1,"integrated_snn_res.0.25"]))
inter2 <- intersect(rownames(immune.integrated@meta.data[as.character(immune.integrated@meta.data[,"assigned_cell_type_byNC"])%in%c("CD14_monocyte","CD16_monocyte"),]),names(cell_type))
cell_type[inter2] <- paste0("immune_",
as.character(immune.integrated@meta.data[inter2,"assigned_cell_type_byNC"]))
sample_info <- cbind(sample_info,cell_type)
sub.integrated$minorLabel <- cell_type
CAF_ids = rownames(sample_info)[sample_info[,"cell_type"]=="cancer_2"] 
malignant_ids = rownames(sample_info)[sample_info[,"cell_type"]%in%c("immune_CD14_monocyte","immune_CD16_monocyte")] 
expressed_genes_sender = expression[CAF_ids,] %>% apply(2,function(x){10*(2**x - 1)}) %>% apply(2,function(x){log2(mean(x) + 1)}) %>% .[. >= 2] %>% names()
expressed_genes_receiver = expression[malignant_ids,] %>% apply(2,function(x){10*(2**x - 1)}) %>% apply(2,function(x){log2(mean(x) + 1)}) %>% .[. >= 1] %>% names()

##Step 2: Define the gene set of interest and a background of genes
mono_marker = FindMarkers(immune.integrated,ident.1 = "TAM_anti_inflammatory",ident.2 =c("CD14_monocyte","CD16_monocyte"), group.by = "assigned_cell_type_byNC" )
geneset_oi = rownames(mono_marker)[(mono_marker[,"avg_log2FC"]>0.8)&(mono_marker[,"p_val_adj"]<0.05)]
background_expressed_genes = expressed_genes_receiver %>% .[. %in% rownames(ligand_target_matrix)]

##Step 3: Define a set of potential ligands
lr_network = lr_network %>% filter(database != "ppi_prediction_go" )
ligands = lr_network %>% pull(from) %>% unique()
expressed_ligands = intersect(ligands,expressed_genes_sender)
receptors = lr_network %>% pull(to) %>% unique()
expressed_receptors = intersect(receptors,expressed_genes_receiver)
...
ligand_activities = predict_ligand_activities(geneset = geneset_oi, background_expressed_genes = background_expressed_genes, ligand_target_matrix = ligand_target_matrix, potential_ligands = potential_ligands)
ligand_activities %>% arrange(-pearson)
best_upstream_ligands = ligand_activities %>% top_n(45, pearson) %>% arrange(-pearson) %>% pull(test_ligand)
...
vis_ligand_receptor_network = lr_network_top_matrix[order_receptors, order_ligands_receptor]
ligands_all = "ANXA1" 
targets_all = c("ATF3","EGR1","CITED2","JUN","MAF","RBPJ","CCL2",
"ESR1","FN1","GSN","LMNA","PLAU","STARD13","HMOX1","ID3","IER2","SQSTM1","TUBB")
t1 <- weighted_networks
t1$lr_sig[(t1$lr_sig[,1]=="ANXA1")&(t1$lr_sig[,2]!="FPR2"),3] <- 0
active_signaling_network = get_ligand_signaling_path(ligand_tf_matrix = ligand_tf_matrix, ligands_all = ligands_all, targets_all = targets_all, weighted_networks = t1,,top_n_regulators = 40)
